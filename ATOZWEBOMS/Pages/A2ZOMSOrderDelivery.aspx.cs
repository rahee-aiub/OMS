...
{
    public partial class A2ZOMSOrderDelivery : System.Web.UI.Page
    {
        ...
        private void TransitPartyDropdown()
        {
            ...
            ddlDeliveryParty = DataAccessLayer.BLL.CommonManager.Instance.FillDropDownList(sqlquery, ddlDeliveryParty, "A2ZACOMS");
        }
        ...
        private void OrderNoDropdown()
        {
            ...
            //string sqlquery = "SELECT AccNo,CONCAT(AccNo,' - ',AccBalance) AS AccInfo FROM A2ZACCOUNT WHERE AccType = 55 AND AccLocation = 3 AND AccBalance > 0";
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                if (ddlDeliveryParty.SelectedIndex == 0)
                {
                    ScriptManager.RegisterStartupScript(this, GetType(), "showalert", "alert('Please Select Delivery Party');", true);
                    return;
                }

                if (txtDeliveryWhomReceive.Text == string.Empty)
                {
                    txtDeliveryWhomReceive.Focus();
                ...
                prm[1] = txtTrnNote.Text;
                prm[2] = txtDeliveryWhomReceive.Text;
                prm[3] = lblID.Text;

                int result = Converter.GetInteger(CommonManager.Instance.GetScalarValueBySp("Sp_UpdateOrderTransaction", prm, "A2ZACOMS"));
                if (result == 0)
                {
                    Response.Redirect(Request.RawUrl);
                }
        ...
        {
            string qry = "SELECT AccBalance FROM A2ZACCOUNT WHERE AccLocation = 3 AND AccNo = " + ddlOrderNo.SelectedValue + "";
        ...
        protected void txtGrossWt_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                var prm = new object[7];
                prm[0] = Converter.GetDateToYYYYMMDD(CtrlProcDate.Text); // TrnDate
                prm[1] = ddlDeliveryParty.SelectedValue; // TrnKeyNo
                prm[2] = ddlOrderNo.SelectedValue; // RefTrnKeyNo
                prm[3] = "4"; // FuncOpt
                prm[4] = "Deliver To Party"; //FuncOptDesc
                prm[5] = txtGrossWt.Text; // TrnAmount 
                prm[6] = lblID.Text; // UserID

                int result = Converter.GetInteger(CommonManager.Instance.GetScalarValueBySp("[Sp_InsertWfOrderDelivery]", prm, "A2ZACOMS"));

                if (result == 0)
                {
                    ddlDeliveryParty.Enabled = false;
