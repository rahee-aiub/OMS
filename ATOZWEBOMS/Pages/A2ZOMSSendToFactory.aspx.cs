...
{
    public partial class A2ZOMSSendToFactory : System.Web.UI.Page
    {
        ...
        private void LoadReceiveGridSpooler()
        {
            ...
            string sqlquery = "SELECT OrderNo,OrderDate,ItemName,ItemKarat,ItemPiece,ItemTotalWeight,A2ZPARTYCODE.PartyName,OrderStatus,OrderStatusDesc FROM A2ZORDERDETAILS inner join A2ZPARTYCODE on A2ZPARTYCODE.PartyCode=A2ZORDERDETAILS.OrderParty WHERE OrderStatus = 11 OR OrderStatus = 13";
        ...
        protected void btnSend_Click(object sender, EventArgs e)
        {
            try
            {
                if (ddlFactoryParty.SelectedIndex == 0)
                {
                    ScriptManager.RegisterStartupScript(this, GetType(), "showalert", "alert('Please Select Factory Name');", true);
                    return;
                }
                ...
                prm[1] = ddlFactoryParty.SelectedValue;
                prm[2] = txtOrderDesc.Text;
                if (CtrlOrdStatus.Text == "11")
                {
                    prm[3] = "12";//OrderStatus
                    prm[4] = "New Order";//OrderStatusDesc;
                }
                else 
                {
                    prm[3] = "14";//OrderStatus
                    prm[4] = "Re-Edit Order";//OrderStatusDesc;
                }

                

                int result = Converter.GetInteger(CommonManager.Instance.GetScalarValueBySp("Sp_OrderSendToFactory", prm, "A2ZACOMS"));
                if (result == 0)
                {
                    Response.Redirect(Request.RawUrl);
                }
